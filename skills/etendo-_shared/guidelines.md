...
| `Authentication failed` | Wrong `bbdd.*` credentials | Check `config/Openbravo.properties` (runtime) and `gradle.properties` (build) — they may differ |
...
1. Check `gradle.properties` for `docker_com.etendoerp.docker_db=true` and `docker_com.etendoerp.tomcat=true` (docker flags are only in gradle.properties). For DB connection params (`bbdd.sid`, `bbdd.user`, etc.), read from `config/Openbravo.properties` (authoritative runtime config)
...
**After editing XML:** Always sort the file with `~/.config/opencode/skills/etendo-_shared/scripts/sort_xml.py` (see section 16b) to keep entries in UUID order.

---

## 16. Reading sourcedata XML files

Etendo stores Application Dictionary data as XML in `src-db/database/sourcedata/`. These files are verbose and hard to read raw. Use the bundled `~/.config/opencode/skills/etendo-_shared/scripts/xml2json.py` to inspect them quickly:

```bash
# List all records (compact table, audit columns hidden)
python3 ~/.config/opencode/skills/etendo-_shared/scripts/xml2json.py SMFWHE_DEFINEDWEBHOOK.xml

# Show specific columns only
python3 ~/.config/opencode/skills/etendo-_shared/scripts/xml2json.py SMFWHE_DEFINEDWEBHOOK.xml --cols NAME,JAVA_CLASS

# Filter by field value (partial, case-insensitive)
python3 ~/.config/opencode/skills/etendo-_shared/scripts/xml2json.py AD_TABLE.xml --filter TABLENAME=smft

# Filter by record ID prefix
python3 ~/.config/opencode/skills/etendo-_shared/scripts/xml2json.py AD_COLUMN.xml --id 0D9B036E
...
python3 ~/.config/opencode/skills/etendo-_shared/scripts/xml2json.py AD_TABLE.xml --json

# Just count records
python3 ~/.config/opencode/skills/etendo-_shared/scripts/xml2json.py AD_COLUMN.xml --count
```
...
Etendo sourcedata XML files must have their entries sorted in **ascending lexicographic order by UUID**. When you add new entries to any XML file (e.g., new `AD_MESSAGE`, `AD_COLUMN`, or `AD_REF_LIST` records), sort the file immediately using the bundled `~/.config/opencode/skills/etendo-_shared/scripts/sort_xml.py`:

```bash
# Sort a single file after adding entries
python3 ~/.config/opencode/skills/etendo-_shared/scripts/sort_xml.py src-db/database/sourcedata/AD_MESSAGE.xml

# Sort multiple files at once
python3 ~/.config/opencode/skills/etendo-_shared/scripts/sort_xml.py src-db/database/sourcedata/AD_MESSAGE.xml src-db/database/sourcedata/AD_REF_LIST.xml

# Sort all sourcedata files in a module
python3 ~/.config/opencode/skills/etendo-_shared/scripts/sort_xml.py path/to/module/src-db/database/sourcedata/*.xml
```
...
See `~/.config/opencode/skills/etendo-_shared/references/known-bugs-webhooks.md` (B6–B9) for details on each issue and their fixes.
...
3. Local `~/.config/opencode/skills/etendo-_shared/references/` files (webhook details, known bugs)
