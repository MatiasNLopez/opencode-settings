...
First, read `~/.config/opencode/skills/etendo-_shared/guidelines.md` and `~/.config/opencode/skills/etendo-_shared/context.md`.
For the full EtendoRX API reference (entities, endpoints, CSRF handling), read `~/.config/opencode/skills/etendo-_shared/references/etendo-headless.md`. For the classic DataSource API and FormInit, read `~/.config/opencode/skills/etendo-_shared/references/etendo-api-guide.md`.
...
If the test returns 401/403: explain SWS access configuration requirement (must create `etrx_rx_services_access` record linking user -> auth service). See `~/.config/opencode/skills/etendo-_shared/references/headless-setup.sql`.
