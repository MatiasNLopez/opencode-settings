...
For DAL patterns, EventHandlers, Background Processes, Callouts, and module file structure, read `~/.config/opencode/skills/etendo-_shared/references/java-development.md`. For Jobs, Actions, and CloneRecordHook, read `~/.config/opencode/skills/etendo-_shared/references/jobs-and-actions.md`.
...
> See `~/.config/opencode/skills/etendo-_shared/references/java-development.md` for advanced callout patterns (e.g., querying the DB inside execute, cascading to multiple fields).
...
After writing any Java code, check it against Etendo's Sonar rules. Read `~/.config/opencode/skills/etendo-_shared/references/java-sonar-rules.md` and verify the generated code does not violate BLOCKER or CRITICAL rules. Key patterns to always check:
